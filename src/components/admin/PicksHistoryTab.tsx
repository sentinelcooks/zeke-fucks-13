...
export const PicksHistoryTab: React.FC<{ password: string }> = ({ password }) => {
  ...
  const handleAutoGrade = async () => {
    ...
    try {
      const { s, e } = presetToDates(preset);
      // PicksHistory covers all non-edge tiers; omit `tier` so grade-picks
      // scans the full pending set minus the safety filters
      // (status != empty_slate, tier != _pending) applied server-side.
