...
export const EdgeHistoryTab: React.FC<{ password: string }> = ({ password }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const reqBody: Record<string, any> = { password, action: "list_edge_history", limit: 500 };
      ...
      const raw = (data?.picks || []) as EdgePick[];
      const filtered = raw.filter(isEdgeHistoryPick);

      const tierDist: Record<string, number> = {};
      const statusDist: Record<string, number> = {};
      for (const p of raw) {
        const t = String(p.tier ?? "null").toLowerCase();
        const s2 = String(p.status ?? "null").toLowerCase();
        tierDist[t] = (tierDist[t] || 0) + 1;
        statusDist[s2] = (statusDist[s2] || 0) + 1;
      }
      console.log("[EdgeHistory] raw count:", raw.length, "filtered:", filtered.length);
      console.log("[EdgeHistory] tier dist:", tierDist, "status dist:", statusDist);
      if (data?.debug) console.log("[EdgeHistory] backend debug:", data.debug);
    ...
    <>
      {error && (
        <div className="rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-200">
          <div className="font-semibold mb-1">Failed to load Edge History</div>
              ...
              <tbody>
                {filtered.map((p) => {
                  ...
                  return (
                    ...
                    console.error("[EdgeHistory] row render failed", p?.id, rowErr);
                    return (
                      <tr key={p.id || Math.random()} className="border-b border-border/50">
                        <td colSpan={12} className="py-2 px-4 text-[11px] text-destructive">
